...
function inPatch(z,x,y){var b=(TILES&&TILES.boxes)||[];
  ...
  return false}
if(SPARSE)maplibregl.addProtocol('apexsat',function(params){
  var m=/apexsat:\/\/(\d+)\/(\d+)\/(\d+)/.exec(params.url||'');
  var blank=function(){return {data:BLANK_PNG.buffer.slice(0)}};
  if(!m||!inPatch(+m[1],+m[2],+m[3]))return Promise.resolve(blank());
  return fetch('bundle/imagery/'+m[1]+'/'+m[2]+'/'+m[3]+'.jpg')
...
var MODES=[
  ...
  {k:'ride',     h:'Ride',     s:'ORV, dirt bike, side-by-side, MTB — trails, riding areas, fuel', act:'ride',
   kinds:['trailhead','camp','fuel','dayuse','view','info','water','toilet','shelter','store','food','mtb'],
   demote:['store','food','info'],
   groups:{areas:true,peaks:false,contour:false,relief:false,paddle:false,places:true,county:false},
   basemap:'Map', zoom:9},
  {k:'outdoors', h:'Outdoors', s:'Hike, camp, hunt, fish — on foot, with hills, counties and rivers', act:'foot', machine:'walk',
   kinds:['trailhead','camp','shelter','water','toilet','view','launch','beach','dayuse','info','system','mtb'],
   peaksFrom:9,
   ...
   groups:{areas:false,peaks:true,contour:true,relief:false,paddle:true,places:true,county:true},
   basemap:'Map', zoom:11},
  {k:'water',    h:'Water',    s:'Beach, kayak, tube, boat — launches and rivers, no trail lines', act:'none',
   ...
   boost:['launch','beach'],
   groups:{areas:false,peaks:false,contour:false,relief:false,paddle:true,places:true,county:false},
...
function applyMode(k,opts){
  ...
  try{
    if(m.machine){if(machine!==m.machine){if(machine!=='walk')rideMachine=machine;
        machine=m.machine;_legalMemo={}}}
    else if(machine==='walk'){machine=rideMachine||'bike';_legalMemo={}}
