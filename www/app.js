...
var map=new maplibregl.Map({container:'map',style:{version:8,glyphs:GLYPH_URL,
  ...
  layers:[
    ...
    /* Roads read as background: a rider is looking for TRAIL. */
    lyr('minor','minor','#4A443B',w(0.4,0.9,2.2)),
    lyr('paved','paved','#3A352E',w(0.9,2,4.8)),
    lyr('track','track','#8C8270',w(0.5,1,2.2),[3,2]),
    lyr('fsroad','fsroad','#6E665B',w(0.7,1.6,4),[4,1.6]),
    ...
    lyr('fstrail','fstrail','#1F6FB2',w(1.2,2.9,7)),
    lyr('trail50','trail50','#1F6FB2',w(1.3,3.2,7.6)),
    lyr('mccct','mccct','#17150F',w(1.3,3.2,7.6)),
    lyr('moto24','moto24','#17150F',w(1.3,3.2,7.6)),
    ...
    {id:'lbl-show',type:'symbol',source:'showonly',minzoom:13.8,
    ...
    {id:'lbl-trail',type:'symbol',source:'strokes',minzoom:12.5,
      filter:['in',['get','c'],['literal',['route72','trail50','moto24','mccct','fstrail']]],
      layout:{'symbol-placement':'line','text-field':['get','lb'],
        'text-font':['APEX'],'text-size':w(10.5,13,16),
        ...
        'text-max-angle':75,'symbol-spacing':100,'text-letter-spacing':0.02,
        'text-padding':2},
    ...
    {id:'lbl-fsroad',type:'symbol',source:'strokes',minzoom:13.4,
      filter:['==',['get','c'],'fsroad'],
      layout:{'symbol-placement':'line','text-field':['get','lb'],
    ...
    {id:'lbl-road',type:'symbol',source:'strokes',minzoom:12.8,
...
function setBasemap(i){
  ...
  LBL.forEach(function(id){
    map.setLayoutProperty(id,'visibility',
      (m==='Satellite'?'none':(el('c-labels').className.indexOf('on')<0?'none':'visible')))});
...
var LBL=['lbl-trail','lbl-fsroad','lbl-road','lbl-place'];
...
function stRender(){
  ...
  stAdd('RENDER','trails',q(['trail50','route72','moto24','fstrail','mccct'])>0,
    q(['trail50','route72','moto24','fstrail','mccct'])+' features');
  ...
  stAdd('RENDER','labels',lb>0,lb+' at '+site[1].toFixed(3)+','+site[0].toFixed(3)+
    ' z13.6 (glyph pack renders)');
...
function refreshReadout(){
  ...
  var r=el('ro-elev');
  if(r)r.textContent=(e===null?'':ft(e)+' ft');
  var v=el('v-elev');
